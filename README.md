...
**Frontend, backend и аналитический CLI объединены.** NestJS принимает три Parquet, запускает Python, проверяет четыре артефакта и выдаёт результат. Python валидирует данные, считает метрики, шесть ролей, кластеры и приоритеты, создаёт три CSV и `analysis.json`. Next.js реализует загрузку, граф, поиск, карточку узла, топ и скачивание. Реальный CLI/API проверен локально и в Docker; 51 frontend-тест проходит. Сквозной браузерный сценарий на настоящих Parquet пройден 23.09.2026: загрузка, граф, три типа узлов, карточки и CSV. Фактические результаты и границы проверки — в [отчёте](docs/BACKEND_VERIFICATION.md). [Инструкция аналитики](analytics/README.md) · [Правила и методология](docs/METHODOLOGY.md).
...
Интеграционная приёмка API/CLI на предоставленных Parquet повторена Ерасылом 23.09.2026 на Windows x64 и в Docker Linux x64: все 2 248 исходных gid сохранены, три CSV доступны. Последний отдельный замер Windows: процесс + backend-валидация **2 289 ms**; браузерный запуск через Docker: **2 384 ms** по состоянию API. В браузере проверены все узлы и направленные рёбра, сохранение 19 изолятов, карточки обычного узла, изолята и depth=4, топ и скачанные CSV. Окружение, команды и границы измерений — в [отчёте](docs/BACKEND_VERIFICATION.md). Данные и результаты исключены из Git; для повторения приёмки набор получают отдельно. Health обновлён и сообщает только доступность HTTP.
...
Правила шести ролей, пороги, формулы, ограничения и масштабирование до ~1 млн узлов описаны в [методологии](docs/METHODOLOGY.md). Сквозная UI-приёмка пройдена, health обновлён, схема и пятиминутный сценарий доступны в [DEMO.md](docs/DEMO.md). К сдаче остаются командная репетиция и сбор разрешённых выгрузок в комплект сдачи отдельно от Git. Измерения на предоставленном наборе не являются проверкой производительности на миллионе узлов. LLM-судьи не подключены и для обязательного расчёта не нужны.
