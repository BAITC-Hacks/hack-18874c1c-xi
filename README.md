...
**Backend и аналитический CLI объединены; интерфейс пока остаётся каркасом.** NestJS принимает три Parquet, запускает Python, проверяет четыре артефакта и выдаёт результат. Python валидирует данные, считает метрики, шесть ролей, кластеры и приоритеты, создаёт три CSV и `analysis.json`. Независимый CLI проверен на предоставленном наборе локально и в Docker. Next.js пока только проверяет доступность NestJS; готовность API и CLI не означает готовность всего решения. [Инструкция аналитики](analytics/README.md) · [Правила и методология](docs/METHODOLOGY.md).
...
Реальные Parquet доступны локально в `data/`, но не включены в Git. Интеграционная приёмка прошла: все 2 248 исходных gid сохранены, три CSV доступны; процесс + backend-валидация заняли 507 ms локально и 644 ms в отдельном HTTP-прогоне Docker. Окружение и границы измерений — в отчёте ниже. Отображение результата в UI требует реализации Ильи и совместной проверки. Старое поле `analytics: "not_implemented"` в health пока сохранено для совместимости с frontend-каркасом; оно не отражает фактический статус CLI и требует согласованного обновления потребителя и контракта.
