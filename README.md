...
**Frontend, backend и аналитический CLI объединены.** NestJS принимает три Parquet, запускает Python, проверяет четыре артефакта и выдаёт результат. Python валидирует данные, считает метрики, шесть ролей, кластеры и приоритеты, создаёт три CSV и `analysis.json`. Next.js реализует загрузку, граф, поиск, карточку узла, топ и скачивание. Полный сценарий на настоящих Parquet прошёл автоматическую приёмку в production Chromium: 7 сквозных сценариев, независимая проверка расчётов и трёх UI-выгрузок. Ручное объяснение трёх gid за минуту и независимый запуск другим участником ещё нужны. [Отчёт и стенд](docs/ACCEPTANCE.md) · [Инструкция аналитики](analytics/README.md) · [Правила и методология](docs/METHODOLOGY.md).
...
Команда создаёт `nodes_roles.csv`, `clusters.csv`, `top_nodes.csv` и внутренний `analysis.json`. Для повторного запуска используй новый каталог, например `run-002`: существующие результаты не перезаписываются. Данные организаторов разложены локально в `data/` и исключены из Git; после нового клонирования их нужно получить отдельно. Описание и starter сохранены в `docs/references/organizers/`. CLI и сквозной API/UI-сценарий проверены на реальном наборе; границы автоматической и ручной приёмки — в [отчёте](docs/ACCEPTANCE.md).
...
Реальные Parquet доступны локально в `data/`, но не включены в Git. Приёмка API/CLI и полный браузерный сценарий прошли: все 2 248 исходных gid сохранены, три CSV доступны и совпадают с прямым расчётом. Полный CLI занял около 0.60 с, UI от нажатия запуска до готового графа — 2.62 с в зафиксированном прогоне; окружение и границы измерений — в [отчёте приёмки](docs/ACCEPTANCE.md). Старое поле `analytics: "not_implemented"` в health не отражает фактический статус CLI и требует обновления; frontend его не использует.
...
Правила шести ролей, пороги, формулы, ограничения и масштабирование до ~1 млн узлов описаны в [методологии](docs/METHODOLOGY.md). К сдаче остаются ручная приёмка трёх произвольных gid, независимое воспроизведение, комплект разрешённых выгрузок, актуальная схема и пятиминутное демо; отдельный технический долг — health. Автоматические тесты не присваивают баллы жюри. LLM-судьи не подключены и для обязательного расчёта не нужны.
