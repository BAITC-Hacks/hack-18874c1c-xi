...
**Текущий результат — стартовый каркас для команды.** Next.js запускается и проверяет доступность NestJS. Python-пакет устанавливается и предоставляет CLI. Расчёт ролей, кластеры, загрузка данных, экспорт и карта анализа ещё не реализованы: endpoints расчёта возвращают явный HTTP 501, CLI не создаёт фиктивные CSV. Успешная проверка окружения не означает готовность решения хакатона.
...
`npm test` проверяет безопасный выбор ветки на изолированных временных репозиториях и HTTP-поведение каркаса API. Python-тесты проверяют ошибки, параметры CLI и отсутствие фиктивных результатов. CI выполняет сборку, проверки структуры, типов и эти тесты; он не заменяет приёмку аналитики на выданных данных.
