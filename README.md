...
**Frontend, backend и аналитический CLI объединены.** NestJS принимает три Parquet, запускает Python, проверяет четыре артефакта и выдаёт результат. Python валидирует данные, считает метрики, шесть ролей, кластеры и приоритеты, создаёт три CSV и `analysis.json`. Next.js реализует загрузку, граф, поиск, карточку узла, топ и скачивание. На 23.09.2026 проходят 51 frontend-тест и 7 сквозных сценариев production-сборки в Chromium на настоящих Parquet, с независимой проверкой расчётов и трёх UI-выгрузок. Отдельные прогоны Ерасыла на Windows и в Docker описаны в [backend-отчёте](docs/BACKEND_VERIFICATION.md); изолированный стенд Родиона — в [отчёте приёмки](docs/ACCEPTANCE.md). Ручное объяснение трёх gid за минуту и воспроизведение итоговой ревизии другим участником ещё нужны. [Инструкция аналитики](analytics/README.md) · [Правила и методология](docs/METHODOLOGY.md).
...
Правила шести ролей, пороги, формулы, ограничения и масштабирование до ~1 млн узлов описаны в [методологии](docs/METHODOLOGY.md). Автоматическая сквозная UI-приёмка пройдена, health обновлён, схема и пятиминутный сценарий доступны в [DEMO.md](docs/DEMO.md). К сдаче остаются командная репетиция с объяснением трёх произвольных gid за минуту, воспроизведение итоговой ревизии другим участником и комплект разрешённых выгрузок отдельно от Git. Автоматические тесты не присваивают баллы жюри; текущие измерения не доказывают производительность на миллионе узлов. LLM-судьи не подключены и для обязательного расчёта не нужны.
