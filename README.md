...
**Frontend, backend и аналитический CLI объединены.** NestJS принимает три Parquet, запускает Python, проверяет четыре артефакта и выдаёт результат. Python валидирует данные, считает метрики, шесть ролей, кластеры и приоритеты, создаёт три CSV и `analysis.json`. Next.js реализует загрузку, граф, поиск, карточку узла, топ и скачивание. Реальный CLI/API проверен локально и в Docker; frontend-валидатор принимает настоящий результат, 51 frontend-тест проходит. Полный пользовательский сценарий в браузере на реальном наборе ещё требует приёмки. [Инструкция аналитики](analytics/README.md) · [Правила и методология](docs/METHODOLOGY.md).
...
Интеграционная приёмка API/CLI на предоставленных Parquet повторена Ерасылом 23.09.2026 на Windows x64 и в Docker Linux x64: все 2 248 исходных gid сохранены, три CSV доступны; процесс + backend-валидация заняли 2 007 ms и 1 927 ms соответственно. Окружение, команды и границы измерений — в отчёте ниже. После проверки три входных Parquet удалены из рабочего каталога по поручению владельца; данные и результаты не включены в Git. Для повторения проверки набор нужно предоставить заново. Интерфейс Ильи включён, но отображение и полный сценарий на реальных данных требуют совместной браузерной проверки. Старое поле `analytics: "not_implemented"` в health не отражает фактический статус CLI и требует обновления; новый frontend его не использует.
...
Правила шести ролей, пороги, формулы, ограничения и масштабирование до ~1 млн узлов описаны в [методологии](docs/METHODOLOGY.md). К сдаче остаётся провести сквозную UI-приёмку, обновить health, включить разрешённые выгрузки в комплект сдачи, актуализировать схему и подготовить пятиминутное демо. LLM-судьи не подключены и для обязательного расчёта не нужны.
