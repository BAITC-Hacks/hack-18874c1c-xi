...
Реальные Parquet доступны локально в `data/`, но не включены в Git. Интеграционная приёмка API/CLI прошла: все 2 248 исходных gid сохранены, три CSV доступны; процесс + backend-валидация заняли 507 ms локально и 644 ms в отдельном HTTP-прогоне Docker. Окружение и границы измерений — в отчёте ниже. Интерфейс Ильи включён, но отображение и полный сценарий на реальных данных требуют совместной браузерной проверки. Старое поле `analytics: "not_implemented"` в health не отражает фактический статус CLI и требует обновления; новый frontend его не использует.
