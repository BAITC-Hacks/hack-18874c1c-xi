...
flowchart LR
    ...
    API -->|Сейчас расчёт отвечает 501| Web
    API -.->|Запуск CLI: ожидает реализации| Python[Python: метрики, роли, кластеры, приоритеты]
    Python -.->|analysis.json и три CSV: ожидают реализации| API
    Fixture[Явный dev-fixture] -->|Только development и fixture=1| Web
```

Frontend уже потребляет контракт статуса, результата и экспортов NestJS. Пунктирные связи на схеме обозначают незавершённый pipeline коллег; Next.js не выполняет его работу. Fixture проверяет только отображение и взаимодействие.
...
Реальные API-ошибки отображаются явно; они не заменяются синтетическим результатом. Работа `/api/health` сама по себе не означает доступность расчёта. В исходном backend расчёт возвращает `501 PIPELINE_NOT_IMPLEMENTED`; завершение реального сценария зависит от реализации API и аналитики коллегами.
...
Отдельно в браузере проверен текущий настоящий NestJS: `POST /api/runs` возвращает HTTP 501, его сообщение отображается, граф и fixture не появляются. Production-сборка успешно проверена с `?fixture=1`: синтетический режим не включается. Production-сборка frontend, общая проверка типов `npm run typecheck`, `team:check`, `check:structure` и `git diff --check` прошли. Системный Node был версии 22; финальная сборка и unit-тесты выполнялись через Node.js 24.21.0 из временного кеша npm, без изменения lockfile или системной установки.
...
## Ожидает интеграции и приёмки

- NestJS должен принимать настоящий комплект файлов, запускать Python, возвращать реальные статусы/результат и отдавать готовые CSV. В проверяемом исходном каркасе pipeline ещё не реализован.
- Аналитик должен предоставить результаты, правила шести ролей, ограничения, значения оценок и три согласованных экспорта. Fixture этих результатов не заменяет.
- Нужна проверка на исходных Parquet: сохранение каждого узла, правильность аналитики, соответствие CSV экрану и фактический полный расчёт менее 300 секунд. Измерений pipeline и подтверждения этих пунктов здесь нет.
...
До появления настоящего pipeline эта репетиция выполняется только частично на явно помеченном fixture. Полный расчёт менее 300 секунд, корректность ролей/кластеров и экспорт на исходном датасете остаются предметом совместной приёмки с backend и аналитикой.
