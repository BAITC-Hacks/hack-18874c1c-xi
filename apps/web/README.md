...
Frontend потребляет контракт статуса, результата и экспортов NestJS. Backend и Python объединены; Next.js не выполняет их расчёты. Проверены отдельные сценарии на fixture и полный браузерный путь через настоящий API с локальными Parquet: 2 248 узлов, 3 119 рёбер, 107 кластеров, топ-20 и три CSV. Результаты и границы проверок приведены ниже и в [отчёте интеграции](../../docs/BACKEND_VERIFICATION.md).
...
Фактически проверено 23.09.2026: 55 frontend unit-тестов прошли на Node.js 24.21.0. Они включают точные ID и суммы, сохранение изолятов, воспроизводимость координат при перестановке входных данных, отсутствие наложения маркеров и компактный вид входящих/исходящих/двусторонних связей. Прошли общая проверка типов, production-сборка Docker, `team:check`, `check:structure` и `git diff --check`.
