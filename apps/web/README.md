...
Frontend потребляет контракт статуса, результата и экспортов NestJS. Backend и Python объединены и проверены на реальных данных; Next.js не выполняет их расчёты. Fixture проверяет только отображение и взаимодействие. На 23.09.2026 прошли 51 frontend-тест, типы, Docker-сборка и 7 production Chromium end-to-end сценариев на настоящих Parquet: 2 248 узлов / 3 119 рёбер / 107 кластеров / топ-20, точный поиск/карточки/направления, скачивание CSV, ошибки и retry. Этот прогон не использует fixture или перехваты API; [отдельный стенд и результаты](../../docs/ACCEPTANCE.md). Объяснение трёх gid человеком за минуту и независимый запуск другим участником остаются ручной приёмкой.
...
- Нужна браузерная приёмка на исходных Parquet: загрузка → граф → поиск узлов, включая изолят и depth=4 → карточка → скачивание; сверка экрана с CSV. Fixture этой проверки не заменяет.
