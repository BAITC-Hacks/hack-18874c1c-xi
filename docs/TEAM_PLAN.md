...
Источник требований: [ТЗ HackAlem AI](https://docs.google.com/document/d/1JPLU-G6R25Ge2hVaY2J9cqvrx7FGExj87XKwJPaMz3o/edit?tab=t.2mrm4atu16r8). Схемы файлов уточняются по README предоставленного датасета. Подготовлен общий каркас и Docker-запуск; аналитические и пользовательские функции ниже остаются планом команды. Актуальное состояние и команды — в корневом README.
...
До готовности реального расчёта допустим маленький явно помеченный dev-fixture по контракту; он не является результатом решения и не заменяет приёмку. Данные и starter сейчас отсутствуют в репозитории: после получения их следует изучить до утверждения порогов. Пока их нет, можно реализовывать контракты, загрузку, интерфейс и синтетические проверки, но нельзя объявить готовым итоговое решение.
